...
namespace Akınsoft
{
    class Program
    {
        static int[] buffer = new int[1000] {6,4,0,7,7,1,7,6,5,3,1,3,3,0,6,2,4,9,1,9,2,2,5,1,1,9,6,7,4,4,2,6,5,7,4,7,4,2,3,5,5,3,4,9,1,9,4,9,3,4,9,6,9,8,3,5,2,0,3,
...
6,1,8,8,2,6,7,0,4,2,8,2,5,2,4,8,3,6,0,0,8,2,3,2,5,7,5,3,0,4,2,0,7,5,2,9,6,3,4,5,0 };
        ...
        {
             var list =Hesapla(buffer,15);
            foreach (var item in list)
            {
                Console.WriteLine("Baslangic index : " + item.baslangic + " Bitis index : " + item.bitis + " Seri :" + item.seri + " Sonuc : " + item.sonuc);
            }
            Console.WriteLine("En yüksek carpım : " + list.Max(x => x.sonuc));
        }
        public static List<Islem> Hesapla(int[]buffer,int sayi)
        {
            int bitis = 0;
            int k = 0;
            int carpım = 1;
            var list = new List<Islem>();
            for (int i = 0; i < buffer.Length; i++)//bu döngü içerisinde aralarında 0 bulunmadan 13 ardışık elemanı buluyoruz
            {
                if (buffer[i] != 0)
                {
                    k++;
                ...
                if (k == (sayi-1))
                {
                    ...
                    islem.baslangic = bitis - (sayi-1);
                    islem.bitis = bitis;
                    for (int j = bitis; j > bitis - (sayi-1); j--)// k== 12 durumunda en büyük elemanlardan biri olma olasılıgı olan sayı dizisini bulmuş oluyoruz ve çarpıyoruz
                    {
                        carpım *= buffer[j];
                        islem.seri += buffer[j] + " , ";
                    }
           ...
           return list;// en büyük çarpımı döndürüyor
        }
    }
}
